...
export interface Level2TraversalSnapshot {
  canvasHeight: number;
  canvasWidth: number;
  playerHeight: number;
  playerWidth: number;
...
function resolveLevel2Bounds(snapshot: Level2TraversalSnapshot): {
  maxX: number;
  maxY: number;
  minX: number;
  minY: number;
} {
  const boundsParity = LEVEL_BOUNDS_PARITY[2];
...
export function canMoveWithinLevel2Bounds(
  snapshot: Level2TraversalSnapshot,
  facingDirection: PlayerFacingDirection,
): boolean {
  const levelBounds = resolveLevel2Bounds(snapshot);
...
export function isLevel2ToLevel3TransitionTriggered(snapshot: Level2TraversalSnapshot): boolean {
  const transitionParity = LEVEL_TRANSITION_BOUNDS_PARITY[2];
