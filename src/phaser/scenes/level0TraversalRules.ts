...
export interface Level0TraversalSnapshot {
  canvasHeight: number;
  canvasWidth: number;
  playerHeight: number;
  playerWidth: number;
...
function resolveLevel0Bounds(snapshot: Level0TraversalSnapshot): {
  maxX: number;
  maxY: number;
  minX: number;
  minY: number;
} {
  const boundsParity = LEVEL_BOUNDS_PARITY[0];
...
export function canMoveWithinLevel0Bounds(
  snapshot: Level0TraversalSnapshot,
  facingDirection: PlayerFacingDirection,
): boolean {
  const levelBounds = resolveLevel0Bounds(snapshot);
...
export function isLevel0ToLevel1TransitionTriggered(snapshot: Level0TraversalSnapshot): boolean {
  const transitionParity = LEVEL_TRANSITION_BOUNDS_PARITY[0];
