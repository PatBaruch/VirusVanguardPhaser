...
export interface Level4TraversalSnapshot {
  canvasHeight: number;
  canvasWidth: number;
  playerHeight: number;
  playerWidth: number;
...
function resolveLevel4Bounds(snapshot: Level4TraversalSnapshot): {
  maxX: number;
  maxY: number;
  minX: number;
  minY: number;
} {
  const boundsParity = LEVEL_BOUNDS_PARITY[4];
...
export function canMoveWithinLevel4Bounds(
  snapshot: Level4TraversalSnapshot,
  facingDirection: PlayerFacingDirection,
): boolean {
  const levelBounds = resolveLevel4Bounds(snapshot);
...
export function isLevel4ToLevel5TransitionTriggered(snapshot: Level4TraversalSnapshot): boolean {
  const transitionParity = LEVEL_TRANSITION_BOUNDS_PARITY[4];
