...
export interface Level3TraversalSnapshot {
  canvasHeight: number;
  canvasWidth: number;
  playerHeight: number;
  playerWidth: number;
...
function resolveLevel3Bounds(snapshot: Level3TraversalSnapshot): {
  maxX: number;
  maxY: number;
  minX: number;
  minY: number;
} {
  const boundsParity = LEVEL_BOUNDS_PARITY[3];
...
export function canMoveWithinLevel3Bounds(
  snapshot: Level3TraversalSnapshot,
  facingDirection: PlayerFacingDirection,
): boolean {
  const levelBounds = resolveLevel3Bounds(snapshot);
...
export function isLevel3ToLevel4TransitionTriggered(snapshot: Level3TraversalSnapshot): boolean {
  const transitionParity = LEVEL_TRANSITION_BOUNDS_PARITY[3];
