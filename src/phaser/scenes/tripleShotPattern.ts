...
export interface TripleShotPatternSnapshot {
...
const TRIPLE_SHOT_PROJECTILE_SPEED_PARITY: number = 2;

export function resolveTripleShotProjectileConfigs(
  snapshot: TripleShotPatternSnapshot,
): ProjectilePrefabConfig[] {
  if (snapshot.levelId !== 4 && snapshot.levelId !== 5) {
    return [];
  }

  const diagonalVelocity: number = TRIPLE_SHOT_PROJECTILE_SPEED_PARITY / Math.sqrt(2);
